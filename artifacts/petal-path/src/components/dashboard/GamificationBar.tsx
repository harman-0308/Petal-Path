...
export default function GamificationBar() {
  ...
  return (
    ...
    >
      <div className="text-[11px] font-bold bg-primary/10 text-primary px-2.5 py-1 rounded-full">
        ✨ {stats.xp} XP
      </div>
      <div className="text-[11px] font-bold bg-secondary/20 text-secondary-foreground px-2.5 py-1 rounded-full">
        Lv. {stats.level} {getTierName(stats.level)}
      </div>
      {xpInLevel > 0 && (
        <div className="flex items-center gap-1.5 bg-muted/30 border border-border/10 rounded-full px-2.5 py-1">
          ...
          <span className="text-[9px] text-muted-foreground font-bold">{xpInLevel}/100</span>
        </div>
      )}
      {stats.streaks.map((s, i) => (
        <div key={i} className="text-[11px] font-bold bg-accent/10 text-accent-foreground px-2.5 py-1 rounded-full">
          {s}
        </div>
      ))}
      {todayBadge && (
        <div className="text-[11px] font-bold bg-yellow-500/10 text-yellow-600 px-2.5 py-1 rounded-full">
          {todayBadge}
        </div>
      )}
    </motion.div>
