...
export default function WeeklyView() {
  const [habits] = useLocalStorage<any[]>("petal-habits", []);
  const [habitLogs] = useLocalStorage<Record<string, string[]>>("petal-habit-logs", {});
  const [sleepLogs] = useLocalStorage<Record<string, any>>("petal-sleep", {});
  ...
  weekDays.forEach(day => {
    ...
    if (sleep && sleep.duration) {
      // rough parse for demo purposes
  ...
  const avgSleep = sleepDays > 0 ? `${Math.floor((totalSleepMins / sleepDays) / 60)}h ${Math.floor((totalSleepMins / sleepDays) % 60)}m` : '0h';
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <Card className="bg-secondary/10 border-none shadow-sm">
          <CardContent className="p-6 flex flex-col items-center justify-center text-center">
            <span className="text-3xl font-bold text-secondary-foreground mb-1">{habits.length > 0 ? Math.max(...habits.map(h => h.streak)) : 0}</span>
            ...
            <table className="w-full text-sm">
                  ...
                  {weekDays.map(day => (
                    <th key={day.toISOString()} className="font-medium text-muted-foreground pb-4 px-2 text-center">
                      <div className="flex flex-col items-center">
                        ...
                        <span className={`w-6 h-6 flex items-center justify-center rounded-full mt-1 ${isSameDay(day, today) ? 'bg-primary text-white' : ''}`}>
                          {format(day, 'd')}
                        </span>
                      </div>
                    </th>
              ...
              <tbody>
                {habits.map((habit, idx) => (
                  ...
                  >
                    <td className="py-3 font-medium text-foreground">{habit.name}</td>
                    {weekDays.map(day => {
                      const dateStr = format(day, "yyyy-MM-dd");
                ...
                {habits.length === 0 && (
                  <tr>
                    <td colSpan={8} className="py-8 text-center text-muted-foreground">
                      No habits to track yet! Add some on the Daily tab.
                    </td>
                  </tr>
                )}
              </tbody>
      ...
      {/* Weekly Planner Placeholder for layout balance */}
      <Card className="bg-card">
        <CardHeader>
        ...
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-3">
            {weekDays.map(day => (
              <div key={day.toISOString()} className="bg-muted/20 rounded-xl p-3 min-h-[120px] border border-border/50">
                <div className="font-bold text-sm text-foreground/80 mb-2">{format(day, 'EEEE')}</div>
                <div className="text-xs text-muted-foreground/60 italic">Notes...</div>
              </div>
            ))}
          </div>
        </CardContent>
